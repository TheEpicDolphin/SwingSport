...
public class Ball : MonoBehaviour
{
    ...
    Vector3 PControllerForce(Vector3 desiredVelocity)
    {
    ...
    Vector3 PControllerWithPredictionForce(Vector3 targetPosition, Vector3 targetVelocity)
    {
        ...
        float homingSpeed = Mathf.Max(minHomingSpeed, 3.0f * targetVelocity.magnitude);
        ...
        else
        {
            ...
            {
                /* Ball needs to move at a faster speed to reach target */
                dt = 0.0f;
            }
            else
            {
        ...
        Vector3 desiredVelocity = homingSpeed * (predictedTargetPosition - transform.position).normalized;
        return PControllerForce(desiredVelocity);
    }
}
